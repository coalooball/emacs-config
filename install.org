* Emacs 开发环境安装

本配置面向 Emacs 30，支持以下开发环境：

- JavaScript、TypeScript 和 React（JSX/TSX）
- Python
- Rust
- C 和 C++

配置使用 Tree-sitter 提供语法解析，Eglot 提供 LSP 功能，Apheleia 在保存后调用对应的格式化工具。

** 1. 前置条件

建议使用 Emacs 30 或更高版本，并确保编译时启用了 Tree-sitter。进入 Emacs 后可以检查：

#+begin_src text
M-x emacs-version
M-: (treesit-available-p)
#+end_src

第二条命令应返回 =t=。

还需要 Git、C/C++ 编译器和 ripgrep。Tree-sitter grammar 会在本机编译，因此必须存在可用的 =cc=。

macOS：

#+begin_src sh
xcode-select --install
brew install ripgrep uv clang-format
brew install --cask font-sarasa-gothic
#+end_src

其中 =font-sarasa-gothic= 提供 =Sarasa Mono SC= 等中文等宽字体，用于改善
Org 表格中中文和英文混排时的显示对齐。安装后重启 GUI Emacs，或重新打开
Emacs frame。

Debian/Ubuntu：

#+begin_src sh
sudo apt update
sudo apt install git ripgrep clangd clang-format build-essential
#+end_src

** 2. 启用配置文件

Emacs 默认从 =~/.emacs.d/init.el= 或 =~/.config/emacs/init.el= 读取配置。可以将仓库中的文件链接到其中一个位置。

macOS 当前仓库路径：

#+begin_src sh
mkdir -p ~/.emacs.d
ln -s /Users/mawangdan/code/emacs-config/init.el ~/.emacs.d/init.el
#+end_src

如果目标位置已经存在 =init.el=，上述 =ln= 命令会停止并报错；应先比较并合并已有配置，不要直接覆盖。Linux 上将源文件路径换成实际的仓库路径。

也可以从命令行临时测试配置：

#+begin_src sh
emacs --init-directory=/Users/mawangdan/code/emacs-config
#+end_src

** 3. 安装 Emacs 包

=init.el= 配置了 MELPA 和 NonGNU ELPA，并通过 =use-package= 自动安装以下包：

- =exec-path-from-shell=
- =vertico=、=orderless=、=marginalia=
- =consult=
- =corfu=、=cape=
- =dockerfile-mode=
- =markdown-mode=
- =treesit-auto=
- =apheleia=
- =magit=
- =diff-hl=

Eglot 已包含在 Emacs 30 中，不需要单独安装。
数据库连接使用 Emacs 内置的 =sql.el=，不需要安装额外 Emacs 包，但需要安装对应
数据库的命令行客户端。

首次启动后，如果包列表尚未更新，执行：

#+begin_src text
M-x package-refresh-contents
#+end_src

然后重新启动 Emacs。需要排查启动错误时，可以运行：

#+begin_src sh
emacs --debug-init
#+end_src

Eat 需要单独安装：

#+begin_src text
M-x package-install RET eat RET
#+end_src

** 4. 安装语言服务器和格式化工具

*** JavaScript、TypeScript 和 React

先安装 Node.js 和 npm，再执行：

#+begin_src sh
npm install -g typescript-language-server typescript prettier
#+end_src

其中：

- =typescript-language-server= 为 JavaScript、TypeScript、JSX 和 TSX 提供 LSP 功能。
- =typescript= 提供 =tsserver=。
- =prettier= 负责保存时格式化 JS、TS、JSX 和 TSX。

React 项目仍应在项目目录安装自己的依赖：

#+begin_src sh
npm install
#+end_src

打开 =.jsx= 文件时使用 =js-ts-mode=，打开 =.tsx= 文件时使用 =tsx-ts-mode=。

*** Python

配置会按顺序寻找以下 Python language server：

- =basedpyright-langserver=
- =pyright-langserver=
- =pylsp=

推荐安装 =basedpyright=：

#+begin_src sh
uv tool install basedpyright
#+end_src

也可以选择用 npm 安装 =pyright=：

#+begin_src sh
npm install -g pyright
#+end_src

或者安装 =python-lsp-server=，它提供 =pylsp=：

#+begin_src sh
uv tool install python-lsp-server
#+end_src

安装 Ruff，用于 Python 格式化：

#+begin_src sh
uv tool install ruff@latest
#+end_src

如果 uv 提示工具目录不在 =PATH= 中，再执行，然后重启 shell 或 GUI Emacs：

#+begin_src sh
uv tool update-shell
#+end_src

Python 项目建议在项目根目录使用 =.venv=：

#+begin_src sh
uv venv
uv sync
#+end_src

*** Rust

通过 rustup 安装 Rust 分析器、格式化工具和标准库源码：

#+begin_src sh
rustup component add rust-analyzer rustfmt rust-src
#+end_src

配置会使用 =rust-analyzer= 提供 LSP 功能，并使用 =rustfmt= 格式化 =.rs= 文件。

*** C 和 C++

需要以下两个命令：

#+begin_src sh
clangd --version
clang-format --version
#+end_src

=clangd= 提供 LSP 功能，=clang-format= 负责保存时格式化。CMake 项目应生成编译数据库，让 clangd 获得准确的 include 路径和编译参数：

#+begin_src sh
cmake -S . -B build -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
#+end_src

clangd 会自动寻找 =build/compile_commands.json=。

** 5. 安装 Tree-sitter grammar

配置只启用了当前需要的 grammar：

- JavaScript、TypeScript、TSX
- Python
- YAML
- Rust
- C、C++

不要在首次打开源文件时下载 grammar；网络很差时，同步下载会卡住 Emacs。
需要安装或补齐 grammar 时，手动执行：

#+begin_src text
M-x cyan/treesit-install-grammars-async
#+end_src

该命令会在单独的 batch Emacs 进程里下载并编译 grammar，当前 Emacs 只显示异步命令输出。安装完成后，可以通过当前 major mode 确认是否生效，例如：

#+begin_src text
M-x describe-mode
#+end_src

对应模式应为 =js-ts-mode=、=typescript-ts-mode=、=tsx-ts-mode=、=python-ts-mode=、=yaml-ts-mode=、=rust-ts-mode=、=c-ts-mode= 或 =c++-ts-mode=。

** 6. 验证开发环境

在 Emacs 中检查外部命令是否都能找到：

#+begin_src elisp
M-: (mapcar #'executable-find
            '("rg"
              "typescript-language-server"
              "ruff"
              "rust-analyzer"
              "rustfmt"
              "clangd"
              "clang-format"
              "prettier"))
M-: (seq-some #'executable-find
              '("basedpyright-langserver" "pyright-langserver" "pylsp"))
#+end_src

第一条结果中不应出现 =nil=。第二条应返回一个可执行文件路径，表示至少安装了一个 Python language server。

打开项目中的源文件后，Eglot 会自动启动。mode line 中应出现 =[eglot:项目名]=。常用检查命令：

#+begin_src text
M-x eglot-events-buffer
M-x apheleia-format-buffer
M-x magit-status
#+end_src

常用快捷键：

| 快捷键 | 功能 |
|---|---|
| =M-.= | 跳转到定义 |
| =M-,= | 返回 |
| =M-?= | 查找引用 |
| =C-s= | 在当前 buffer 搜索 |
| =M-s r= | 使用 ripgrep 搜索项目 |
| =C-x b= | 切换 buffer |
| =C-x g= | 打开 Magit |

** 7. 数据库连接

本配置使用 Emacs 内置的 =sql.el= 连接数据库。Emacs 负责编辑 SQL 和管理交互
buffer，真正连接数据库的是本机命令行客户端：

| 数据库 | 需要的命令 |
|---|---|
| PostgreSQL | =psql= |
| MySQL/MariaDB | =mysql= |
| SQLite | =sqlite3= |

macOS 可以按需安装：

#+begin_src sh
brew install postgresql@16 mysql sqlite
#+end_src

Debian/Ubuntu 可以按需安装：

#+begin_src sh
sudo apt install postgresql-client mysql-client sqlite3
#+end_src

配置里已经提供了三个连接模板：

- =local-postgres=：连接 =localhost:5432= 的 =postgres= 数据库
- =local-mysql=：连接 =localhost:3306=
- =local-sqlite=：打开 =~/database.sqlite3=

按自己的环境修改 =init.el= 里的 =sql-connection-alist= 即可。例如 PostgreSQL：

#+begin_src elisp
("local-postgres"
 (sql-product 'postgres)
 (sql-user "postgres")
 (sql-server "localhost")
 (sql-port 5432)
 (sql-database "postgres"))
#+end_src

不要把密码写进 =init.el=。PostgreSQL 建议用 =~/.pgpass=，MySQL/MariaDB 建议用
=~/.my.cnf=，或者让客户端在连接时提示输入密码。

常用入口：

| 快捷键 / 命令 | 功能 |
|---|---|
| =C-c d c= | 选择 =sql-connection-alist= 中的连接 |
| =C-c d p= | 设置当前 SQL buffer 的数据库类型 |
| =M-x sql-postgres= | 手动连接 PostgreSQL |
| =M-x sql-mysql= | 手动连接 MySQL/MariaDB |
| =M-x sql-sqlite= | 手动连接 SQLite |
| =C-c C-r= | 在 =.sql= 文件中发送选中区域 |
| =C-c C-b= | 发送整个 SQL buffer |
| =C-c C-c= | 发送当前段落 |
| =C-c C-z= | 跳转到 SQL 交互 buffer |

使用方式：

1. 打开或新建一个 =.sql= 文件。
2. 执行 =C-c d c=，选择一个连接，例如 =local-postgres=。
3. 在 SQL 文件里写查询，选中语句后按 =C-c C-r= 发送到交互 buffer。

也可以不先写文件，直接执行 =M-x sql-postgres=、=M-x sql-mysql= 或
=M-x sql-sqlite= 进入交互式 SQL buffer。

** 8. 使用 Org mode

Org mode 已包含在 Emacs 中，本配置不需要额外安装包。Org 文件默认保存在
=~/.emacs.d/org/=；如果使用 =--init-directory= 启动本仓库，则保存在仓库的
=org/= 目录中。第一次启动时会自动创建该目录。

常用入口：

| 快捷键 | 功能 |
|---|---|
| =C-c c t= | 快速记录任务到 =inbox.org= |
| =C-c c n= | 快速记录笔记到 =notes.org= |
| =C-c a a= | 查看 Agenda |
| =C-c l= | 保存当前位置的 Org 链接 |
| =C-c C-t= | 在 Org 标题上切换任务状态 |
| =C-c C-s= | 为任务安排日期 |
| =C-c C-d= | 为任务设置截止日期 |

例如在 =inbox.org= 中记录一个带截止日期的任务：

#+begin_src org
* TODO 完成项目说明
  DEADLINE: <2026-08-05 Wed>
#+end_src

=org-agenda-files= 默认包含整个 Org 目录，因此该目录下的所有 =.org= 文件都会
进入 Agenda。任务完成时会自动记录完成时间。

** 9. Eat 的 DEL 键异常

*** 现象

在 Eat 终端中按下 =DEL=（Backspace）键时，光标反而向右移动，且没有删除字符。

*** 原因

Eat 的 terminfo 数据库没有正确编译，导致 shell 输出的终端控制序列未被正确处理。

*** 解决方法

在 Emacs 中执行：

#+begin_src text
M-x eat-compile-terminfo
#+end_src

关闭已有的 Eat buffer，然后重新执行 =M-x eat=。重新打开后，=DEL= 键即可正常删除光标前的字符。

可以在新的 Eat 终端中检查 terminfo 是否生效：

#+begin_src sh
echo "$TERM"
infocmp "$TERM" >/dev/null && echo OK
#+end_src

=TERM= 应类似于 =eat-truecolor=，并且第二条命令应输出 =OK=。
